...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let dat = try! contract.generateDataParameter(toAddress: "0x376D1a3e3E71de213CDD835AC54416796Ca0Ce00", amount: "2222")
        ...
        geth.getTransactionCount(of: address) { result in
            ...
            case .success(let nonce):
                let wei: BInt
                do {
                    wei = try Converter.toWei(ether: "0")
                } catch let error {
                    fatalError("Error: \(error.localizedDescription)")
                }
                
//                let rawTransaction = RawTransaction(value: wei, to: address, gasPrice: Converter.toWei(GWei: 10), gasLimit: 21000, nonce: nonce)
                let rawTransaction = RawTransaction(wei: "0", to: contract.contractAddress, gasPrice: Converter.toWei(GWei: 10), gasLimit: 600000, nonce: nonce, data: dat)
                let tx: String
                do {
